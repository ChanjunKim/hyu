...
data = np.loadtxt('./tensor_test.csv', delimiter=',', unpack=True, dtype='float32')

x_source = np.transpose(data[0:2])
y_source = np.transpose(data[2:])
...
softmax_logits = tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=model)
...
with tf.Session() as sess:
    ...
    for step in range(2):
        ...
        print(step+1, sess.run([global_step, cost], feed_dict={X: x_source, Y: y_source}))
